...
chains_dir = "/tigress/ljchang/dSph_likelihoods/RunFiles/multinest_chains/lina_mocks/"

if not os.path.exists(chains_dir):
	os.makedirs(chains_dir)
plots_dir_base = "/tigress/ljchang/dSph_likelihoods/Plots/light_profile_plots/lina_mocks/"
data_folder_base = "/tigress/ljchang/dSph_likelihoods/Generate_mocks/mocks/"

extra_text = "a_1_b_3_g_1_rdm_1_iso"
has_truth = True
...
for rs in rs_list:
	for rho0 in rho0_list:
		for nstars in [1000]:
			data_tag_base = "dist_as_2.0_bs_5.0_gs_0.1_rs_"+str(rs)+"_dm_"+str(nstars)+"_"
			true_params = [0,np.log10(rs),2,5,0.1]	

			for iMC in range(10):
			# for iMC in [0]:
				data_tag = data_tag_base+str(iMC)+"_rho_"+str(rho0)
				data_file_path = data_folder_base+extra_text+"/"+data_tag+"_no_err_lp.npz"

				save_dir = chains_dir+extra_text+"/"
				quantiles_dir = data_folder_base+extra_text+"/light_profile_bestfits/"
				...
				plots_dir = plots_dir_base+extra_text+"/"+data_tag+"/"
