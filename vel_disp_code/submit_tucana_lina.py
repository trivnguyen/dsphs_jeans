...
chains_dir_base = "/tigress/lnecib/dSph_likelihoods/RunFiles/vel_disp_code/multinest_chains/"
...
post_dir_base = "/tigress/lnecib/dSph_likelihoods/RunFiles/vel_disp_code/DM_posteriors/"
batch_dir = "/tigress/lnecib/dSph_likelihoods/RunFiles/batch/"
temp_directory = "/tigress/lnecib/dSph_likelihoods/RunFiles/temp/"
...
run_tag_initial = "_measured_verr"
	...
	for nu_model, lp_tag in nu_model_list:
		for dm_model in dm_model_list:		
			for nbeta in nbeta_list:
				...
				batch4 = "mpiexec -np 10 python scan_vel_disp_interface.py --run_scan $run_scan --chains_dir $chains_dir --data_file_path $data_file_path --temp_directory $temp_directory --light_profile_params_path $light_profile_params_path --mock $mock "
				...
				batch6 = "python scan_vel_disp_interface.py --cleanup_dirs --chains_dir $chains_dir --temp_ind $temp_ind --mock $mock --temp_directory $temp_directory --make_plots $make_plots --plots_dir $plots_dir --data_file_path $data_file_path"+"\n"
				batch7 = "python scan_vel_disp_interface.py --make_plots $make_plots --save_plots $save_plots --save_post $save_post --mock $mock --temp_directory $temp_directory "
				...
				if run_scan and make_plots:
					...
					fname = batch_dir + "/scan_plot_"+extratext+"_"+data_tag+"_"+nu_model+"_"+dm_model+"_"+str(nbeta)+run_tag+".batch"
				elif run_scan:
					batchn = batch1+batch2+batch3+batch4+batch5+batch6
					fname = batch_dir + "/scan_"+extratext+"_"+data_tag+"_"+nu_model+"_"+dm_model+"_"+str(nbeta)+run_tag+".batch"
				elif make_plots:
					batchn = batch1+batch2+batch7+batch8+batch9
					fname = batch_dir + "/plot_"+extratext+"_"+data_tag+"_"+nu_model+"_"+dm_model+"_"+str(nbeta)+run_tag+".batch"
