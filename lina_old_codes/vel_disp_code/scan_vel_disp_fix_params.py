...
class run_scan():
	def __init__(self, 
				 true_params,
				 data_file_path = "",
				 light_profile_params_path = "", true_profile = "PlumCuspIso",
				 load_light_profile = True, fix_light_profile = False, 
				 nu_model = "Plummer", dm_model = "gNFW", fix_breaks = False,
				 nbeta = 0, deltav = 0.2, measured_errs = False, verbose = True,
				 run_tag = "", fix_tag=""):
	...
	def set_priors(self):
		...
		if self.dm_model == "BPL":
			...
			if self.fix_tag == "fix_rho0_r0":
				# Fix rho0, r0
				self.fixed_params['fix_inds'] = self.fixed_params['fix_inds']+[self.nstarparams_theory,self.nstarparams_theory+1]
				self.fixed_params['fix_vals'] = self.fixed_params['fix_vals']+self.true_params[:2]
				self.params = self.params + [r'$\gamma$']
				self.priors = self.priors + [[-1,5]]
				self.ndmparams = 1
				print("Fixing logrho0 to ",str(self.true_params[0]), ", fixing logr0 to ",str(self.true_params[1]))

			elif self.fix_tag == "fix_rho0":
				# Fix rho0
				self.fixed_params['fix_inds'] = self.fixed_params['fix_inds']+[self.nstarparams_theory]
				self.fixed_params['fix_vals'] = self.fixed_params['fix_vals']+[self.true_params[0]]
				self.params = self.params + [r'$\log(r_0)$', r'$\gamma$']
				self.priors = self.priors + [[-10,10],[-1,5]]
				self.ndmparams = 2
				print("Fixing logrho0 to ",str(self.true_params[0]))

			elif self.fix_tag == "fix_r0":
				# Fix r0
				self.fixed_params['fix_inds'] = self.fixed_params['fix_inds']+[self.nstarparams_theory+1]
				self.fixed_params['fix_vals'] = self.fixed_params['fix_vals']+[self.true_params[1]]
				self.params = self.params + [r'$\log(\rho_0)$', r'$\gamma$']
				self.priors = self.priors + [[5,30],[-1,5]]
				self.ndmparams = 2
				print("Fixing logr0 to ",str(self.true_params[1]))
