...
parser.add_argument("--full_file_path", action="store", dest="full_file_path", default="", type=str)
...
parser.add_argument("--calculate_j_factor", action="store", dest="calculate_j_factor", default=False, type=ast.literal_eval) 
parser.add_argument("--distance", action="store", dest="distance", default=50., type=float) 
parser.add_argument("--angle_max", action="store", dest="angle_max", default=0.5, type=float) 
parser.add_argument("--full_posterior", action="store", dest="full_posterior", default=False, type=ast.literal_eval) 
parser.add_argument("--mock", action="store", dest="mock", default=False, type=ast.literal_eval) 
...
full_file_path = results.full_file_path
...
calculate_j_factor = results.calculate_j_factor
distance = results.distance
angle_max = results.angle_max
full_posterior = results.full_posterior
mock = results.mock
...
"load_light_profile":load_light_profile, \
"fix_light_profile":fix_light_profile, \
"nu_model":nu_model, \
"dm_model":dm_model, \
...
if run_scan:
	scan = scan_vel_disp_multinest.run_scan(**args)
	...
	if len(chains_dir) < 78: # Account for (len(base)+len(post_equal_weights.dat))<100
		scan.perform_scan_multinest(chains_dir=chains_dir)
...
if calculate_j_factor:
	scan = scan_vel_disp_multinest.run_scan(**args)
	jfac = scan.calculate_j_factor(distance = distance, angle_max = angle_max, chains_dir = chains_dir, full_posterior = full_posterior, post_dir=post_dir)	
